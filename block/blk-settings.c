...
int blk_stack_limits(struct queue_limits *t, struct queue_limits *b,
  ...
  t->max_integrity_segments =
      min_not_zero(t->max_integrity_segments, b->max_integrity_segments);
  ...
  t->raid_partial_stripes_expensive =
      max(t->raid_partial_stripes_expensive, b->raid_partial_stripes_expensive);
  ...
  if (b->discard_granularity) {
    ...
    if (t->discard_granularity != 0 && t->discard_alignment != alignment) {
    ...
    t->max_discard_sectors =
        min_not_zero(t->max_discard_sectors, b->max_discard_sectors);
    t->max_hw_discard_sectors =
        min_not_zero(t->max_hw_discard_sectors, b->max_hw_discard_sectors);
    t->discard_granularity =
        max(t->discard_granularity, b->discard_granularity);
    t->discard_alignment =
        lcm_not_zero(t->discard_alignment, alignment) % t->discard_granularity;
  }

  if (b->chunk_sectors)
    t->chunk_sectors = min_not_zero(t->chunk_sectors, b->chunk_sectors);
...
int blk_queue_dma_drain(struct request_queue *q,
                        dma_drain_needed_fn *dma_drain_needed, void *buf,
                        unsigned int size) {
