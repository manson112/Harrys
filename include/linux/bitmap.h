...
// 목적 데이터의 각 비트를 0으로 초기화
static inline void bitmap_zero(unsigned long *dst, unsigned int nbits) {
  // dst : 목적 데이터
  // nbits : 처리할 비트 수
  // small_const_nbits : CPU가 한번의 operation으로 처리할 수 있는지 여부 판단
  if (small_const_nbits(nbits))
    // 한번에 처리할 수 있으면 dst에 0 대입 / 빠름
    *dst = 0UL;
  else {
    // 한번에 처리 불가
    // 필요한 길이를 구한 후 memset으로 처리/ 느림
...
#define BITMAP_FROM_U64(n)                                                     \
  ((unsigned long)((u64)(n)&ULONG_MAX)), ((unsigned long)((u64)(n) >> 32))
#endif
