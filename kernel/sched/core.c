...
static inline int select_task_rq(struct task_struct *p, int cpu, int sd_flags,
   ...
   */
  if (unlikely(!cpumask_test_cpu(cpu, &p->cpus_allowed) || !cpu_online(cpu)))
...
#ifdef CONFIG_PROC_SYSCTL
int sysctl_schedstats(struct ctl_table *table, int write, void __user *buffer,
...
void preempt_notifier_inc(void) { static_branch_inc(&preempt_notifier_key); }
EXPORT_SYMBOL_GPL(preempt_notifier_inc);

void preempt_notifier_dec(void) { static_branch_dec(&preempt_notifier_key); }
...
int sched_setattr_nocheck(struct task_struct *p,
                          const struct sched_attr *attr) {
