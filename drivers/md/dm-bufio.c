...
static void __insert(struct dm_bufio_client *c, struct dm_buffer *b) {
  ...
  while (*new) {
    ...
    parent = *new;
    new = (found->block < b->block) ? &((*new)->rb_left) : &((*new)->rb_right);
  }
...
static void *alloc_buffer_data(struct dm_bufio_client *c, gfp_t gfp_mask,
  ...
  if (c->block_size <= KMALLOC_MAX_SIZE && gfp_mask & __GFP_NORETRY) {
    *data_mode = DATA_MODE_GET_FREE_PAGES;
    return (void *)__get_free_pages(gfp_mask, c->sectors_per_block_bits -
 ...
 * Free buffer's data.
 */
static void free_buffer_data(struct dm_bufio_client *c, void *data,
                             unsigned char data_mode) {
  switch (data_mode) {
  case DATA_MODE_SLAB:
  ...
  default:
    DMCRIT("dm_bufio_free_buffer_data: bad data mode: %d", data_mode);
    BUG();
  }
}
...
static void use_bio(struct dm_buffer *b, int rw, sector_t sector,
  ...
  do {
    ...
    if (!bio_add_page(bio, virt_to_page(ptr), this_step, offset_in_page(ptr))) {
      bio_put(bio);
      goto dmio;
    }
...
static void submit_io(struct dm_buffer *b, int rw,
                      void (*end_io)(struct dm_buffer *, blk_status_t)) {
  unsigned n_sectors;
  sector_t sector;
  unsigned offset, end;
...
static void write_endio(struct dm_buffer *b, blk_status_t status) {
  ...
  if (unlikely(status)) {
    ...
    (void)cmpxchg(&c->async_write_error, 0, blk_status_to_errno(status));
  }
 ...
 */
static struct dm_buffer *
__alloc_buffer_wait_no_callback(struct dm_bufio_client *c, enum new_flag nf) {
  struct dm_buffer *b;
  ...
  while (1) {
    ...
    if (dm_bufio_cache_size_latch != 1 && !tried_noio_alloc) {
      dm_bufio_unlock(c);
      b = alloc_buffer(c, GFP_NOIO | __GFP_NORETRY | __GFP_NOMEMALLOC |
                              __GFP_NOWARN);
      dm_bufio_lock(c);
      if (b)
        return b;
    ...
    if (!list_empty(&c->reserved_buffers)) {
      b = list_entry(c->reserved_buffers.next, struct dm_buffer, lru_list);
      list_del(&b->lru_list);
...
static void __check_watermark(struct dm_bufio_client *c,
  ...
  while (c->n_buffers[LIST_CLEAN] + c->n_buffers[LIST_DIRTY] > limit_buffers) {
...
found_buffer:
  ...
  b->hold_count++;
  __relink_lru(b,
               test_bit(B_DIRTY, &b->state) || test_bit(B_WRITING, &b->state));
  return b;
}
...
void dm_bufio_prefetch(struct dm_bufio_client *c, sector_t block,
  ...
  for (; n_blocks--; block++) {
    int need_submit;
    struct dm_buffer *b;
    b = __bufio_new(c, block, NF_PREFETCH, &need_submit, &write_list);
...
void dm_bufio_release(struct dm_buffer *b) {
  ...
  if (!b->hold_count) {
     ...
     * invalid buffer.
     */
    if ((b->read_error || b->write_error) && !test_bit(B_READING, &b->state) &&
        !test_bit(B_WRITING, &b->state) && !test_bit(B_DIRTY, &b->state)) {
      __unlink_buffer(b);
      __free_buffer_wake(b);
    }
  }
...
void dm_bufio_mark_partial_buffer_dirty(struct dm_buffer *b, unsigned start,
                                        unsigned end) {
  struct dm_bufio_client *c = b->c;
...
sector_t dm_bufio_get_block_number(struct dm_buffer *b) { return b->block; }
EXPORT_SYMBOL_GPL(dm_bufio_get_block_number);

void *dm_bufio_get_block_data(struct dm_buffer *b) { return b->data; }
EXPORT_SYMBOL_GPL(dm_bufio_get_block_data);

void *dm_bufio_get_aux_data(struct dm_buffer *b) { return b + 1; }
...
static bool __try_evict_buffer(struct dm_buffer *b, gfp_t gfp) {
  if (!(gfp & __GFP_FS)) {
    if (test_bit(B_READING, &b->state) || test_bit(B_WRITING, &b->state) ||
...
static unsigned long dm_bufio_shrink_count(struct shrinker *shrink,
  ...
  unsigned long count =
      READ_ONCE(c->n_buffers[LIST_CLEAN]) + READ_ONCE(c->n_buffers[LIST_DIRTY]);
  unsigned long retain_target = get_retain_buffers(c);
...
dm_bufio_client_create(struct block_device *bdev, unsigned block_size,
  ...
  if (block_size <= KMALLOC_MAX_SIZE &&
    ...
    c->slab_cache =
        kmem_cache_create(slab_name, c->block_size, ARCH_KMALLOC_MINALIGN,
  ...
  c->slab_buffer =
      kmem_cache_create(slab_name, sizeof(struct dm_buffer) + aux_size, 0,
                        SLAB_RECLAIM_ACCOUNT, NULL);
  if (!c->slab_buffer) {
    r = -ENOMEM;
    goto bad;
...
bad:
  while (!list_empty(&c->reserved_buffers)) {
    struct dm_buffer *b =
        list_entry(c->reserved_buffers.next, struct dm_buffer, lru_list);
    list_del(&b->lru_list);
    free_buffer(b);
  }
...
static void __evict_old_buffers(struct dm_bufio_client *c,
...
static void __exit dm_bufio_exit(void) {
  ...
  if (dm_bufio_client_count) {
    DMCRIT("%s: dm_bufio_client_count leaked: %d", __func__,
           dm_bufio_client_count);
    bug = 1;
  }

  if (dm_bufio_current_allocated) {
    DMCRIT("%s: dm_bufio_current_allocated leaked: %lu", __func__,
           dm_bufio_current_allocated);
    bug = 1;
  }

  if (dm_bufio_allocated_get_free_pages) {
    DMCRIT("%s: dm_bufio_allocated_get_free_pages leaked: %lu", __func__,
           dm_bufio_allocated_get_free_pages);
    bug = 1;
  }

  if (dm_bufio_allocated_vmalloc) {
    DMCRIT("%s: dm_bufio_vmalloc leaked: %lu", __func__,
           dm_bufio_allocated_vmalloc);
    bug = 1;
  }
...
module_init(dm_bufio_init) module_exit(dm_bufio_exit)

    module_param_named(max_cache_size_bytes, dm_bufio_cache_size, ulong,
                       S_IRUGO | S_IWUSR);
...
module_param_named(retain_bytes, dm_bufio_retain_bytes, ulong,
                   S_IRUGO | S_IWUSR);
...
module_param_named(allocated_get_free_pages_bytes,
                   dm_bufio_allocated_get_free_pages, ulong, S_IRUGO);
