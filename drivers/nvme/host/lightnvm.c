...
static inline void _nvme_nvm_check_size(void) {
  ...
  BUILD_BUG_ON(sizeof(struct nvme_nvm_chk_meta) != sizeof(struct nvm_chk_meta));
}
...
static int nvme_nvm_setup_12(struct nvme_nvm_id12 *id, struct nvm_geo *geo) {
...
static int nvme_nvm_setup_20(struct nvme_nvm_id20 *id, struct nvm_geo *geo) {
...
static int nvme_nvm_identity(struct nvm_dev *nvmdev) {
  ...
  ret = nvme_submit_sync_cmd(ns->ctrl->admin_q, (struct nvme_command *)&c, id,
  ...
  case 1:
    ...
    break;
  case 2:
    ret = nvme_nvm_setup_20((struct nvme_nvm_id20 *)id, &nvmdev->geo);
    break;
  default:
...
static int nvme_nvm_get_bb_tbl(struct nvm_dev *nvmdev, struct ppa_addr ppa,
  ...
  ret = nvme_submit_sync_cmd(ctrl->admin_q, (struct nvme_command *)&c, bb_tbl,
                             tblsz);
  if (ret) {
...
static int nvme_nvm_set_bb_tbl(struct nvm_dev *nvmdev, struct ppa_addr *ppas,
  ...
  ret = nvme_submit_sync_cmd(ns->ctrl->admin_q, (struct nvme_command *)&c, NULL,
                             0);
  if (ret)
...
static int nvme_nvm_get_chk_meta(struct nvm_dev *ndev,
                                 struct nvm_chk_meta *meta, sector_t slba,
                                 int nchks) {
  ...
  while (left) {
    ...
    ret = nvme_get_log_ext(ctrl, ns, NVME_NVM_LOG_REPORT_CHUNK, dev_meta, len,
                           offset);
    if (ret) {
...
static int nvme_nvm_submit_user_cmd(struct request_queue *q, struct nvme_ns *ns,
                                    struct nvme_nvm_command *vcmd,
                                    void __user *ubuf, unsigned int bufflen,
                                    void __user *meta_buf,
                                    unsigned int meta_len, void __user *ppa_buf,
                                    unsigned int ppa_len, u32 *result,
                                    u64 *status, unsigned int timeout) {
  ...
  if (ubuf && bufflen) {
    ...
    if (meta_buf && meta_len) {
      metadata = dma_pool_alloc(dev->dma_pool, GFP_KERNEL, &metadata_dma);
      if (!metadata) {
        ret = -ENOMEM;
        goto err_map;
      }
      ...
      if (write) {
        if (copy_from_user(metadata, (void __user *)meta_buf, meta_len)) {
          ret = -EFAULT;
          goto err_meta;
        }
      }
...
static int nvme_nvm_submit_vio(struct nvme_ns *ns,
  ...
  ret = nvme_nvm_submit_user_cmd(
      ns->queue, ns, &c, (void __user *)(uintptr_t)vio.addr, length,
      (void __user *)(uintptr_t)vio.metadata, vio.metadata_len,
      (void __user *)(uintptr_t)vio.ppa_list, vio.nppas, &vio.result,
      &vio.status, 0);
...
static int nvme_nvm_user_vcmd(struct nvme_ns *ns, int admin,
  ...
  ret = nvme_nvm_submit_user_cmd(
      q, ns, (struct nvme_nvm_command *)&c, (void __user *)(uintptr_t)vcmd.addr,
      vcmd.data_len, (void __user *)(uintptr_t)vcmd.metadata, vcmd.metadata_len,
      (void __user *)(uintptr_t)vcmd.ppa_list, vcmd.nppas, &vcmd.result,
      &vcmd.status, timeout);
...
void nvme_nvm_unregister(struct nvme_ns *ns) { nvm_unregister(ns->ndev); }
...
static ssize_t nvm_dev_attr_show(struct device *dev,
  ...
  if (strcmp(attr->name, "version") == 0) {
    ...
    return scnprintf(page, PAGE_SIZE, "Unhandled attr(%s) in `%s`\n",
                     attr->name, __func__);
  }
}
...
static ssize_t nvm_dev_attr_show_ppaf(struct nvm_addrf_12 *ppaf, char *page) {
  return scnprintf(
      page, PAGE_SIZE, "0x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x%02x\n",
      ppaf->ch_offset, ppaf->ch_len, ppaf->lun_offset, ppaf->lun_len,
      ppaf->pln_offset, ppaf->pln_len, ppaf->blk_offset, ppaf->blk_len,
      ppaf->pg_offset, ppaf->pg_len, ppaf->sec_offset, ppaf->sec_len);
}
...
void nvme_nvm_unregister_sysfs(struct nvme_ns *ns) {
  ...
  switch (geo->major_ver_id) {
  case 1:
    sysfs_remove_group(&disk_to_dev(ns->disk)->kobj, &nvm_dev_attr_group_12);
    break;
  case 2:
    sysfs_remove_group(&disk_to_dev(ns->disk)->kobj, &nvm_dev_attr_group_20);
    break;
  }
}
