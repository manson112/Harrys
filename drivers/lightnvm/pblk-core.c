...
static void pblk_mark_bb(struct pblk *pblk, struct pblk_line *line,
  ...
  *ppa = ppa_addr;
  pblk_gen_run_ws(pblk, NULL, ppa, pblk_line_mark_bb, GFP_ATOMIC, pblk->bb_wq);
}
...
static void pblk_bio_map_addr_endio(struct bio *bio) { bio_put(bio); }
...
static int pblk_line_init_bb(struct pblk *pblk, struct pblk_line *line,
  ...
  while ((bit = find_next_bit(line->blk_bitmap, lm->blk_per_line, bit + 1)) <
         lm->blk_per_line) {
    off = bit * geo->ws_opt;
    bitmap_shift_left(l_mg->bb_aux, l_mg->bb_template, off, lm->sec_per_line);
...
static int pblk_prepare_new_line(struct pblk *pblk, struct pblk_line *line) {
  ...
  for (i = 0; i < lm->blk_per_line; i++) {
    ...
    if (state & NVM_CHK_ST_FREE) {
      set_bit(pblk_ppa_to_pos(geo, rlun->bppa), line->erase_bitmap);
      blk_to_erase--;
    }
  }
...
struct pblk_line *pblk_line_replace_data(struct pblk *pblk) {
  ...
  //다음 data line
  new = l_mg->data_next;
  //다음 line이 null이면 return new;
  if (!new)
    goto out;

  //현재 data_line을 new로 바꿈
  l_mg->data_line = new;

  spin_lock(&l_mg->free_lock);
  //새 line의 meta data 설정
...
retry_erase:
  ...
  if (left_seblks) {
    ...
    // line에 지워지지않은 블록이 있다면
    if (atomic_read(&new->left_eblks)) { // line 삭제
...
struct pblk_line *pblk_line_get_data(struct pblk *pblk) {
  // pblk => line management의 data_line
...
static void pblk_save_lba_list(struct pblk *pblk, struct pblk_line *line) {
  ...
  w_err_gc->lba_list =
      pblk_malloc(lba_list_size, l_mg->emeta_alloc_type, GFP_KERNEL);
  memcpy(w_err_gc->lba_list, emeta_to_lbas(pblk, emeta->buf), lba_list_size);
}
