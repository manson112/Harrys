...
// PBLK_H_ 가 정의되지 않았을 경우 PBLK_H_ 정의
#ifndef PBLK_H_
#define PBLK_H_
...
enum {
  /* IO Types */
  PBLK_IOTYPE_USER = 1 << 0, // 1
  PBLK_IOTYPE_GC = 1 << 1,   // 2

  /* Write buffer flags */
  PBLK_FLUSH_ENTRY = 1 << 2,     // 4
  PBLK_WRITTEN_DATA = 1 << 3,    // 8
  PBLK_SUBMITTED_ENTRY = 1 << 4, // 16
  PBLK_WRITABLE_ENTRY = 1 << 5,  // 32
};
...
struct pblk_line {
  ...
  int type; /* PBLK_LINETYPE_X */   // line type
  int gc_group; /* PBLK_LINEGC_X */ // gc group
  struct list_head list;            /* Free, GC lists */

  // line에 map된 lun bitmap
...
static inline int pblk_ppa_to_line(struct ppa_addr p) { return p.a.blk; }
...
static inline struct ppa_addr pblk_ppa32_to_ppa64(struct pblk *pblk,
                                                  u32 ppa32) {
  ...
  if (ppa32 == -1) {
    ...
    if (geo->version == NVM_OCSSD_SPEC_12) {
      ...
      ppa64.m.grp = (ppa32 & lbaf->ch_mask) >> lbaf->ch_offset;
      ppa64.m.pu = (ppa32 & lbaf->lun_mask) >> lbaf->lun_offset;
      ppa64.m.chk = (ppa32 & lbaf->chk_mask) >> lbaf->chk_offset;
      ppa64.m.sec = (ppa32 & lbaf->sec_mask) >> lbaf->sec_offset;
    }
  }
...
static inline u32 pblk_ppa64_to_ppa32(struct pblk *pblk,
                                      struct ppa_addr ppa64) {
  u32 ppa32 = 0;
...
static inline u32 pblk_calc_smeta_crc(struct pblk *pblk,
  ...
  crc = crc32_le(
      crc, (unsigned char *)smeta + sizeof(struct line_header) + sizeof(crc),
      lm->smeta_len - sizeof(struct line_header) - sizeof(crc));
...
static inline u32 pblk_calc_emeta_crc(struct pblk *pblk,
  ...
  crc = crc32_le(
      crc, (unsigned char *)emeta + sizeof(struct line_header) + sizeof(crc),
      lm->emeta_len[0] - sizeof(struct line_header) - sizeof(crc));
...
#ifdef CONFIG_NVM_DEBUG
static inline void print_ppa(struct nvm_geo *geo, struct ppa_addr *p, char *msg,
                             int error) {
  if (p->c.is_cached) {
    pr_err("ppa: (%s: %x) cache line: %llu\n", msg, error, (u64)p->c.line);
  } else if (geo->version == NVM_OCSSD_SPEC_12) {
    pr_err("ppa: (%s: %x):ch:%d,lun:%d,blk:%d,pg:%d,pl:%d,sec:%d\n", msg, error,
           p->g.ch, p->g.lun, p->g.blk, p->g.pg, p->g.pl, p->g.sec);
  } else {
    pr_err("ppa: (%s: %x):ch:%d,lun:%d,chk:%d,sec:%d\n", msg, error, p->m.grp,
           p->m.pu, p->m.chk, p->m.sec);
  }
}
  ...
  for (i = 0; i < nr_ppas; i++) {
    ...
    if (geo->version == NVM_OCSSD_SPEC_12) {
      if (!ppa->c.is_cached && ppa->g.ch < geo->num_ch &&
          ppa->g.lun < geo->num_lun && ppa->g.pl < geo->num_pln &&
          ppa->g.blk < geo->num_chk && ppa->g.pg < geo->num_pg &&
          ppa->g.sec < geo->ws_min)
        continue;
    } else {
      if (!ppa->c.is_cached && ppa->m.grp < geo->num_ch &&
          ppa->m.pu < geo->num_lun && ppa->m.chk < geo->num_chk &&
          ppa->m.sec < geo->clba)
        continue;
    }
