...
// pos= 리스트의 헤드가 가리키는 리스트가 있는 구조체 ; 다시 자기 자신에게 돌아오기 전까지(리스트는 순환구조이므로) ;  pos를 리스트의 next가 가리키는 있는 구조체로 바꿔줌
...
// ptr(구조체 내부의 임의의 포인터 값)을 가지고 구조체의 주소를 찾아 type*(type: ptr을 멤버로 갖는 자료구조 형)으로 반환
// member : 구조체 내에서 ptr의 멤버 이름
...
//  };
	...
	//주어진 nvm_dev를 포함하는 struct인 nvm_target의 이름과  주어진 이름이 같다면 해당 nvm_target을 반환하고 없다면 아니면 NULL을 반환
...
// 일반적으로 P와 V 함수 쌍을 사용하는데, linux 에서는 P함수는 “down”, V 함수를 “up”이라 부른다. 
// 단일 세마포어(공유 자원 개수를 1개로 정의)로 사용할 때 뮤텍스(Mutual Exclusion)라 부른다.
...
static bool nvm_target_exists(const char *name)
{
	...
	list_for_each_entry(dev, &nvm_devices, devices) {
		...
		list_for_each_entry(tgt, &dev->targets, list) {
			//dev의 target list에 있는 각각의 target의 name과 주어진 name이 같다면 true
			if (!strcmp(name, tgt->disk->disk_name)) {
				ret = true;
...
// 해당 비트를 셋 하고(1), 해당 비트가 1(set) 이었으면 1 리턴, 해당 비트가 0(clear) 이었으면 0 리턴

//lun을 reserve 하는데 주어진 lun_begin ~ lun_end 사이에서 이미 set 된 비트가 있다면 다시 set했던 비트를 clear 하고 -EBUSY 리턴 
...
static void nvm_release_luns_err(struct nvm_dev *dev, int lun_begin, int lun_end)
{
	int i;
...
//nvm_tgt_dev 생성
static struct nvm_tgt_dev *nvm_create_tgt_dev(struct nvm_dev *dev, u16 lun_begin, u16 lun_end, u16 op)
{
	...
	//GFP_ATOMIC : 커널에 할당 가능한 메모리가 있으면 무조건 할당, 없으면 즉시 NULL 반환, 
	//              프로세스가 잠드는 경우는 없지만 할당에 실패한 경우를 대비해 예외처리 필수
	//GFP_DMA : 연속된 물리 메모리를 할당받을 때 사용 (물리적 공간이 나뉘어 있으면 DMA컨트롤러에서는 사용할 수 없음)
...
static int nvm_config_check_luns(struct nvm_geo *geo, int lun_begin, int lun_end)
{
	//lun_begin과 lun_end가 맞지 않거나 lun_end의 범위가 모든 lun범위보다 크면 오류를 출력한다.
...
static int __nvm_config_simple(struct nvm_dev *dev, struct nvm_ioctl_create_simple *s)
{
...
static int __nvm_config_extended(struct nvm_dev *dev, struct nvm_ioctl_create_extended *e)
{
...
// 	if (!q)
// 		return NULL;
//  (ida_simple_get : 여러개의 request queue가 있을 때 각각의 고유한 id를 만들어주는 함수) 
...
static void nvm_ppa_tgt_to_dev(struct nvm_tgt_dev *tgt_dev, struct ppa_addr *ppa_list, int nr_ppas)
{
	int i;
