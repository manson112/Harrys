...
next_rq:
  ...
  nr_secs = 0;
  do {
    bit = find_next_zero_bit(invalid_bitmap, lm->sec_per_line, bit + 1);
...
static struct pblk_line *pblk_gc_get_victim_line(struct pblk *pblk,
  ...
  // gc group list의 첫 entry가 victim
  victim = list_first_entry(group_list, struct pblk_line, list);
  list_for_each_entry(line, group_list, list) {
    line_vsc = le32_to_cpu(*line->vsc);
    victim_vsc = le32_to_cpu(*victim->vsc);
    // vsc가 가장 작은 line이 victim이 된다.
...
void pblk_gc_free_full_lines(struct pblk *pblk) {
  ...
  do {
    ...
    // Free, GC list의 첫번째 line을 가져온다
    line = list_first_entry(&l_mg->gc_full_list, struct pblk_line, list);

    spin_lock(&line->lock);
    WARN_ON(line->state != PBLK_LINESTATE_CLOSED);
    // line state를 PBLK_LINESTATE_GC로 변경
    line->state = PBLK_LINESTATE_GC;
    spin_unlock(&line->lock);

    // GC list에서 해당 line 삭제
    list_del(&line->list);
    spin_unlock(&l_mg->gc_lock);
...
static void pblk_gc_run(struct pblk *pblk) {
  ...
  // werr line이 있거나 (gc가 active 상태이고 필요한 line 수가 free인 line 수
  // 보다 작을 때 true
  run_gc = pblk_gc_should_run(&pblk->gc, &pblk->rl);
  /* Queue depth for inflight GC lines */
  if (!run_gc || (atomic_read(&gc->read_inflight_gc) >= PBLK_GC_L_QD))
    return;

  // gc_group : gc_high_list, gc_mid_list, gc_log_list, gc_werr_list
next_gc_group:
  ...
  do {
    ...
    // gc list 에서 현재 line 제거
    list_del(&line->list);
    spin_unlock(&l_mg->gc_lock);

    spin_lock(&gc->r_lock);
    // gc->r_list에 line->list(gc list)를 추가
...
void pblk_gc_should_kick(struct pblk *pblk) { pblk_rl_update_rates(&pblk->rl); }
...
int pblk_gc_init(struct pblk *pblk) {
  ...
  gc->gc_writer_ts =
      kthread_create(pblk_gc_writer_ts, pblk, "pblk-gc-writer-ts");
  ...
  gc->gc_reader_ts =
      kthread_create(pblk_gc_reader_ts, pblk, "pblk-gc-reader-ts");
   ...
   */
  gc->gc_line_reader_wq =
      alloc_workqueue("pblk-gc-line-reader-wq", WQ_MEM_RECLAIM | WQ_UNBOUND,
                      PBLK_GC_MAX_READERS);
  ...
  gc->gc_reader_wq =
      alloc_workqueue("pblk-gc-line_wq", WQ_MEM_RECLAIM | WQ_UNBOUND, 1);
  if (!gc->gc_reader_wq) {
