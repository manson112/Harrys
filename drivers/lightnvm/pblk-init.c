...
static int pblk_rw_io(struct request_queue *q, struct pblk *pblk,
  ...
  /* NVMe의 64 비트 완성 비트 맵 제약으로 인해
   * 읽기 요청은 256kb 미만이어야합니다.
   * 쓰기는 임의 크기 일 수 있습니다.
   */
  // #define bio_data_dir(bio)	((bio)->bi_rw & 1)
  // data direction을 반환한다. Read 인지 Write인지
...
static int pblk_set_addrf(struct pblk *pblk) {
  ...
  case NVM_OCSSD_SPEC_20:
    pblk->addrf_len =
        pblk_set_addrf_20(geo, (void *)&pblk->addrf, &pblk->uaddrf);
    break;
  default:
...
static int pblk_init_global_caches(struct pblk *pblk) {
  ...
  pblk_ws_cache =
      kmem_cache_create("pblk_blk_ws", sizeof(struct pblk_line_ws), 0, 0, NULL);
  if (!pblk_ws_cache) {
    up_write(&pblk_lock);
    return -ENOMEM;
  }
  ...
  pblk_rec_cache =
      kmem_cache_create("pblk_rec", sizeof(struct pblk_rec_ctx), 0, 0, NULL);
  if (!pblk_rec_cache) {
  ...
  pblk_g_rq_cache = kmem_cache_create("pblk_g_rq", pblk_g_rq_size, 0, 0, NULL);
  if (!pblk_g_rq_cache) {
    kmem_cache_destroy(pblk_ws_cache);
  ...
  pblk_w_rq_cache = kmem_cache_create("pblk_w_rq", pblk_w_rq_size, 0, 0, NULL);
  if (!pblk_w_rq_cache) {
    kmem_cache_destroy(pblk_ws_cache);
    kmem_cache_destroy(pblk_rec_cache);
...
static int pblk_core_init(struct pblk *pblk) {
  ...
  pblk->pad_dist =
      kzalloc((pblk->min_write_pgs - 1) * sizeof(atomic64_t), GFP_KERNEL);
  if (!pblk->pad_dist)
    return -ENOMEM;
  ...
  pblk->gen_ws_pool =
      mempool_create_slab_pool(PBLK_GEN_WS_POOL_SIZE, pblk_ws_cache);
  if (!pblk->gen_ws_pool)
  ...
  pblk->close_wq = alloc_workqueue("pblk-close-wq", WQ_MEM_RECLAIM | WQ_UNBOUND,
                                   PBLK_NR_CLOSE_JOBS);
  if (!pblk->close_wq)
    goto free_w_rq_pool;

  pblk->bb_wq = alloc_workqueue("pblk-bb-wq", WQ_MEM_RECLAIM | WQ_UNBOUND, 0);
  if (!pblk->bb_wq)
    goto free_close_wq;

  pblk->r_end_wq =
      alloc_workqueue("pblk-read-end-wq", WQ_MEM_RECLAIM | WQ_UNBOUND, 0);
  if (!pblk->r_end_wq)
    goto free_bb_wq;
...
static unsigned int calc_emeta_len(struct pblk *pblk) {
  ...
  lm->emeta_sec[1] =
      DIV_ROUND_UP(sizeof(struct line_emeta) + lm->blk_bitmap_len +
                       sizeof(struct wa_counters),
                   geo->csecs);
...
add_emeta_page:
  ...
  if (geo->all_luns > 1)
    lm->min_blk_line +=
        DIV_ROUND_UP(lm->smeta_sec + lm->emeta_sec[0], geo->clba);
...
static void *pblk_init(struct nvm_tgt_dev *dev, struct gendisk *tdisk,
  ...
  pr_info("pblk(%s): luns:%u, lines:%d, secs:%llu, buf entries:%u\n",
          tdisk->disk_name, geo->all_luns, pblk->l_mg.nr_lines,
          (unsigned long long)pblk->rl.nr_secs, pblk->rwb.nr_entries);
