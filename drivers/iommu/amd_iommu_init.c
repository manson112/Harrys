 ...
 *
 * These data structures are laid over the table to parse the important
 * values out of it.
 */
...
static inline unsigned long tbl_size(int entry_size) {
  unsigned shift =
      PAGE_SHIFT + get_order(((int)amd_iommu_last_bdf + 1) * entry_size);

  return 1UL << shift;
}

int amd_iommu_get_num_iommus(void) { return amd_iommus_present; }
...
static void iommu_enable_ppr_log(struct amd_iommu *iommu) {
  ...
  memcpy_toio(iommu->mmio_base + MMIO_PPR_LOG_OFFSET, &entry, sizeof(entry));
...
static int iommu_init_ga_log(struct amd_iommu *iommu) {
  ...
  iommu->ga_log =
      (u8 *)__get_free_pages(GFP_KERNEL | __GFP_ZERO, get_order(GA_LOG_SIZE));
  if (!iommu->ga_log)
    goto err_out;

  iommu->ga_log_tail =
      (u8 *)__get_free_pages(GFP_KERNEL | __GFP_ZERO, get_order(8));
  if (!iommu->ga_log_tail)
    goto err_out;

  entry = iommu_virt_to_phys(iommu->ga_log) | GA_LOG_SIZE_512;
  memcpy_toio(iommu->mmio_base + MMIO_GA_LOG_BASE_OFFSET, &entry,
              sizeof(entry));
  entry = (iommu_virt_to_phys(iommu->ga_log) & 0xFFFFFFFFFFFFFULL) & ~7ULL;
  memcpy_toio(iommu->mmio_base + MMIO_GA_LOG_TAIL_OFFSET, &entry,
              sizeof(entry));
...
static bool copy_device_table(void) {
  ...
  old_dev_tbl_cpy =
      (void *)__get_free_pages(gfp_flag, get_order(dev_table_size));
 ...
 */
static void __init set_dev_entry_from_acpi(struct amd_iommu *iommu, u16 devid,
...
static int __init add_acpi_hid_device(u8 *hid, u8 *uid, u16 *devid,
  ...
  list_for_each_entry(entry, list, list) {
    if (strcmp(entry->hid, hid) ||
        (*uid && *entry->uid && strcmp(entry->uid, uid)) || !entry->cmd_line)
      continue;
...
static int __init add_early_maps(void) {
  ...
  for (i = 0; i < early_ioapic_map_size; ++i) {
    ret = add_special_device(IVHD_SPECIAL_IOAPIC, early_ioapic_map[i].id,
  ...
  for (i = 0; i < early_hpet_map_size; ++i) {
    ret = add_special_device(IVHD_SPECIAL_HPET, early_hpet_map[i].id,
  ...
  for (i = 0; i < early_acpihid_map_size; ++i) {
    ret = add_acpi_hid_device(
        early_acpihid_map[i].hid, early_acpihid_map[i].uid,
        &early_acpihid_map[i].devid, early_acpihid_map[i].cmd_line);
    if (ret)
      return ret;
  }
 ...
 * it
 */
static void __init set_device_exclusion_range(u16 devid,
  ...
  while (p < end) {
    ...
    case IVHD_DEV_ALIAS:
      ...
      DUMP_printk("  DEV_ALIAS\t\t\t devid: %02x:%02x.%x "
                  ...
                  PCI_BUS_NUM(e->devid), PCI_SLOT(e->devid), PCI_FUNC(e->devid),
                  e->flags, PCI_BUS_NUM(e->ext >> 8), PCI_SLOT(e->ext >> 8),
                  PCI_FUNC(e->ext >> 8));
    ...
    case IVHD_DEV_RANGE_END:
      ...
      DUMP_printk("  DEV_RANGE_END\t\t devid: %02x:%02x.%x\n",
                  PCI_BUS_NUM(e->devid), PCI_SLOT(e->devid),
                  PCI_FUNC(e->devid));
    ...
    case IVHD_DEV_SPECIAL: {
      ...
      DUMP_printk("  DEV_SPECIAL(%s[%d])\t\tdevid: %02x:%02x.%x\n", var,
                  (int)handle, PCI_BUS_NUM(devid), PCI_SLOT(devid),
                  PCI_FUNC(devid));
...
static void amd_iommu_ats_write_check_workaround(struct amd_iommu *iommu) {
  ...
  if ((boot_cpu_data.x86 != 0x15) || (boot_cpu_data.x86_model < 0x30) ||
 ...
 */
static int __init init_iommu_one(struct amd_iommu *iommu,
                                 struct ivhd_header *h) {
  int ret;
  ...
  case 0x10:
    ...
    if ((h->efr_attr != 0) && ((h->efr_attr & (0xF << 13)) != 0) &&
  ...
  while (p < end) {
    ...
    if (*p == amd_iommu_target_ivhd_type) {
      ...
      DUMP_printk("device: %02x:%02x.%01x cap: %04x "
                  ...
                  PCI_BUS_NUM(h->devid), PCI_SLOT(h->devid), PCI_FUNC(h->devid),
                  h->cap_ptr, h->pci_seg, h->flags, h->info);
...
static void init_iommu_perf_ctr(struct amd_iommu *iommu) {
  ...
  if ((iommu_pc_get_set_reg(iommu, 0, 0, 0, &val, true)) ||
      (iommu_pc_get_set_reg(iommu, 0, 0, 0, &val2, false)) || (val != val2)) {
...
static int iommu_init_pci(struct amd_iommu *iommu) {
  ...
  if (iommu_feature(iommu, FEATURE_GT) && iommu_feature(iommu, FEATURE_PPR)) {
  ...
  if (is_rd890_iommu(iommu->dev)) {
    ...
    iommu->root_pdev = pci_get_domain_bus_and_slot(0, iommu->dev->bus->number,
                                                   PCI_DEVFN(0, 0));
  ...
  iommu_device_sysfs_add(&iommu->iommu, &iommu->dev->dev, amd_iommu_groups,
                         "ivhd%d", iommu->index);
...
static void print_iommu_info(void) {
  static const char *const feat_str[] = {"PreF", "PPR", "X2APIC", "NX", "GT",
                                         "[5]",  "IA",  "GA",     "HE", "PC"};
  struct amd_iommu *iommu;
...
static int __init amd_iommu_init_pci(void) {
  ...
  for_each_iommu(iommu) iommu_flush_all_caches(iommu);
...
static int __init init_unity_map_range(struct ivmd_header *m) {
  ...
  DUMP_printk("%s devid_start: %02x:%02x.%x devid_end: %02x:%02x.%x"
              ...
              s, PCI_BUS_NUM(e->devid_start), PCI_SLOT(e->devid_start),
              PCI_FUNC(e->devid_start), PCI_BUS_NUM(e->devid_end),
              PCI_SLOT(e->devid_end), PCI_FUNC(e->devid_end), e->address_start,
              e->address_end, m->flags);
...
static void iommu_apply_resume_quirks(struct amd_iommu *iommu) {
  ...
  pci_write_config_dword(iommu->dev, iommu->cap_ptr + 4, iommu->stored_addr_lo);
  pci_write_config_dword(iommu->dev, iommu->cap_ptr + 8, iommu->stored_addr_hi);
...
static void early_enable_iommus(void) {
  ...
  if (!copy_device_table()) {
    ...
    if (old_dev_tbl_cpy != NULL)
      free_pages((unsigned long)old_dev_tbl_cpy, get_order(dev_table_size));
...
static void disable_iommus(void) {
  ...
  for_each_iommu(iommu) iommu_disable(iommu);
...
static void amd_iommu_resume(void) {
  ...
  for_each_iommu(iommu) iommu_apply_resume_quirks(iommu);
...
static void __init free_iommu_resources(void) {
  ...
  free_pages((unsigned long)irq_lookup_table, get_order(rlookup_table_size));
...
static int __init early_amd_iommu_init(void) {
  ...
  amd_iommu_dev_table = (void *)__get_free_pages(
      GFP_KERNEL | __GFP_ZERO | GFP_DMA32, get_order(dev_table_size));
  ...
  amd_iommu_alias_table =
      (void *)__get_free_pages(GFP_KERNEL, get_order(alias_table_size));
  ...
  amd_iommu_pd_alloc_bitmap = (void *)__get_free_pages(
      GFP_KERNEL | __GFP_ZERO, get_order(MAX_DOMAIN_ID / 8));
  ...
  if (amd_iommu_irq_remap) {
    ...
    amd_iommu_irq_cache = kmem_cache_create("irq_remap_cache", remap_cache_sz,
                                            IRQ_TABLE_ALIGNMENT, 0, NULL);
    if (!amd_iommu_irq_cache)
      goto out;

    irq_lookup_table = (void *)__get_free_pages(GFP_KERNEL | __GFP_ZERO,
                                                get_order(rlookup_table_size));
...
struct amd_iommu *get_amd_iommu(unsigned int idx) {
  ...
  for_each_iommu(iommu) if (i++ == idx) return iommu;
  return NULL;
}
...
static int iommu_pc_get_set_reg(struct amd_iommu *iommu, u8 bank, u8 cntr,
  ...
  if ((offset < MMIO_CNTR_REG_OFFSET) || (offset > max_offset_lim))
    return -EINVAL;
